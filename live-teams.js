(()=>{
  ...
  function injectUI(){
    ...
    document.getElementById('liveAddTeamBtn').onclick=openModal;
    document.getElementById('cancelLiveTeam').onclick=closeModal;
    document.getElementById('confirmLiveTeam').onclick=addLiveTeam;
  ...
  function openModal(){
    ...
    const input=document.getElementById('liveTeamName');
    input.value='';
    const used=new Set((state.teams||[]).map(t=>t.color));
    selectedColor=colors.find(c=>!used.has(c))||colors[(state.teams||[]).length%colors.length];
    document.querySelectorAll('.live-color-btn').forEach(b=>b.classList.toggle('selected',b.dataset.color===selectedColor));
    document.getElementById('liveTeamModal').classList.remove('hidden');
  ...
  async function addLiveTeam(){
    ...
    if((state.teams||[]).length>=maxLiveTeams)return toast(`Maximum ${maxLiveTeams} teams in a session`);
    cleanPriority();
    const prioritySet=new Set(state.newTeamPriority);
    let insertAt=0;
    while(insertAt<state.queue.length&&prioritySet.has(state.queue[insertAt].id))insertAt++;
    const team={id:crypto.randomUUID(),name,color:selectedColor||colors[state.teams.length%colors.length],games:0,wins:0,draws:0,streak:0,bestStreak:0,joinedLive:true};
    state.teams.push(team);
    state.queue.splice(insertAt,0,team);
    state.newTeamPriority.push(team.id);
    closeModal();
    render();
    await push();
    toast(`${name} added — playing next`);
  }
  ...
  render=function(){
    ...
    if(role==='host'){
      cleanPriority();
      document.querySelectorAll('.queue-item').forEach((row,i)=>{
        const team=state.queue[i];
        if(team&&state.newTeamPriority?.includes(team.id)){
          const nameEl=row.querySelector('.queue-name');
          if(nameEl&&!nameEl.querySelector('.new-team-badge'))nameEl.insertAdjacentHTML('beforeend',' <span class="new-team-badge">NEW</span>');
        }
      });
    }
  };
